        ...
        <div>
                        ...
                        setup(picker) {
                            picker.on('view', e => {
                                ...
                                if (view === 'CalendarDay' && availableDates[dateString]) {
                                    ...
                                    span.innerHTML = `${availableDates[dateString]} slots`
                                    target.append(span)
                                }
                            })
                        }
            ...
            <div
                x-data="{
                    slots: [],
                    fetchSlots(event) {
                        const url = `{{ route('slots', [$employee, $service]) }}?date=${form.date}`;
                        axios.get(url).then((response) => this.slots = response.data.times);
                    }
                }"
                    ...
                    <div class="grid grid-cols-3 md:grid-cols-5 gap-8 mt-6">
                        <template x-for="slot in slots">
                            <div
                                ...
                                x-bind:class="{ 'bg-slate-200 hover:bg-slate-200': form.time == slot }"
                            ></div>
                        </template>
                    </div>
                </div>
