<x-app-layout>
    <div>
        <a href="{{route('bookings')}}" class="text-sm text-blue-500">&larr; Go back</a>
        <h2 class="text-xl font-medium">Now choose a service from {{ $employee->name }}</h2>
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-8 mt-6">
            @foreach($services as $service)
                <a href="{{ route('checkout', [$employee, $service]) }}" class="py-8 px-4 border border-slate-200 rounded-lg shadow-sm flex flex-col items-center justify-center text-center hover:bg-gray-50/75">
                    ...
                    <div class="text-sm font-medium text-slate-400 mt1">{{ $service->duration }} minutes</div>
