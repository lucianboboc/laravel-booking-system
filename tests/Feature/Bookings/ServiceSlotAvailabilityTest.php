...
it('shows available time slots for a service', function () {
    ...
    expect($availability->first()->slots)
        ->toHaveCount(16);
});
...
it('excludes booked appointments for the employee', function () {
    ...
    $slots = $availability->map(fn (Date $date) =>
        $date->slots->map(fn (Slot $slot) => $slot->time->toTimeString())
    )->flatten();
...
it('ignore cancelled appointment', function () {
    Carbon::setTestNow(Carbon::parse('1st January 2000'));
